// Page copy — Tokyo · venue scout · CONFIG + TYPE_META + VENUES
const CONFIG = {
  kicker: "Tokyo Tech Week · Venue Scout",
  title: "Tokyo Tech Week — Venue Options",
  subtitle: "Event-capable cafes, coworking & event halls in Shibuya, Nihonbashi/Kayabacho, and Roppongi — for a bilingual (JP/EN) Sept 9, 2026 tech-week coffee/hack event (~50–200 people). Links point to each venue's verified private-events pages or booking contacts.",
  extraStat: { n: "Sep 9", l: "Tokyo event date, 2026" },
  footnotes: [
    "<b>Neighborhood analysis (JP/EN event history):</b> Shibuya is Tokyo's proven bilingual tech hub (Builders Weekend 2026, ai& × Moonshot, Tokyo Tech Meetup); Nihonbashi/Kayabacho and Roppongi are strong enterprise/startup or sponsor-backed expansions. Akihabara/Odaiba is the next cell — Flutter-Hackathon-ready for larger 200-person formats.",
    "<b>Gone since last tech week:</b> Tsutaya BOOK APARTMENT Shinjuku (out of scope for event hire), TCDC Cafe Roppongi (permanently closed), several corporate cafeterias in Marunouchi/Otemachi (members-only drop-ins). See evidence log for the full closure list.",
  ],
  provenance: "Researched & links verified Aug 6, 2026 · Bilingual (JP/EN) queries used throughout · Photos load from each venue's own website; placeholder appears where no photo was published. Capacities are published or estimated figures — confirm when booking.",
};

const TYPE_META = {
  coffee: { label: "Coffee shop", icon: "☕" },
  cowork: { label: "Coworking / event space", icon: "🏢" },
};

const VENUES = [
  {
    "k": "or_miyashita_park",
    "name": "OR | Miyashita Park (オア・ミヤシタパーク)",
    "area": "Shibuya",
    "type": "coffee",
    "addr": "MIYASHITA PARK North 1F 11000, 6-20-10 Jingumae, Shibuya-ku, Tokyo 150-0001",
    "cap": "<b>1</b>F Cafe & Bar 100 / 2F Art Gallery 100 / 3F Music Bar + Event Space 300 (up to ~500 across floors, combined rental possible)",
    "flag": "dedicated",
    "link": "https://www.ortokyo.com/en/rental",
    "linkText": "EN Rental (EVENT RENTAL / PRIVATE PARTY / POP-UP)",
    "email": null,
    "note": "Bilingual JP/EN dedicated rental page with 3 rentable floors sized exactly at 100/100/300, published hourly rates, equipment list and charter forms. Ground-floor cafe bar serves barista coffee all day, escalator links straight to Miyashita Park for outdoor hack-day overflow; PA/DJ/lighting + dedicated staff make it the strongest tech-week fit. Phone 03-6712-5277 listed for large inquiries.",
    "img": "https://www.ortokyo.com/wp-content/themes/or-renewal/images/index/top/00_AJAJAJAJ.jpg",
    "alt": "OB Miyashita Park atrium event floor",
    "top": true
  },
  {
    "k": "jinnan_cafe_shibuya",
    "name": "JINNAN CAFE Shibuya (神南カフェ 渋谷店)",
    "area": "Shibuya",
    "type": "coffee",
    "addr": "Coula Bldg 1F/B1F, 1-17-5 Jinnan, Shibuya-ku, Tokyo 150-0041 (JR Shibuya Hachiko Gate, 7 min walk)",
    "cap": "B1 only: <b>50</b>seated / 80 standing; whole venue (2 floors + terrace): 80 seated / 120 standing; 150 total seats (48 on 1F / 78 on B1 / 24 terrace)",
    "flag": "dedicated",
    "link": "https://jinnancafe.com/shibuya",
    "linkText": "Official site (貸切可 + Party menu + FAQ covering corporate promotion use)",
    "email": null,
    "note": "Coffee-forward loft cafe in Jinnan with its own 貸切 FAQ confirming corporate/Promotion use, projector, mic/PA/screen/stage all on-site, 24h option (advance arrangement) and Wi-Fi. B1 (~100 seats at 120 standing capacity) plus 1F + terrace gives natural breakout zones for a hack-day; bookings via 03-5728-3786 or reservations hotline 050-1720-1399.",
    "img": "https://jinnancafe.com/shibuya/wp-content/uploads/sites/2/2022/12/渋谷店店舗トップ加工.jpg",
    "alt": "Jinnan Cafe Shibuya loft cafe",
    "top": false
  },
  {
    "k": "eight_space",
    "name": "eight space (エイトスペース, ハラカド裏 コワーキングカフェ)",
    "area": "Shibuya",
    "type": "coffee",
    "addr": "6-33-14 Jingumae, Shibuya-ku, Tokyo 150-0001 (2 min from Meiji-jingumae / behind Harakado)",
    "cap": "Event Space <b>50</b>(with streaming gear), whole-venue buyout to ~60; Cafe Lounge 30 seats, Meeting rooms 4–8",
    "flag": "dedicated",
    "link": "https://8sp.jp",
    "linkText": "Official rental + events page (cafe / coworking / event space under one roof)",
    "email": "rental@8sp.jp",
    "note": "Startup-incubation cafe that already runs monthly pitch nights and hands-on AI workshops for 20–40 pax, so the tech-week format is native. Wi-Fi + power at all seats, 100-inch projector + 5.1ch audio + streaming gear + kitchen, cafe space ¥19,800/h and event space listed for up to 60 with revenue-share sponsorship option.",
    "img": null,
    "alt": "Eight space coworking cafe interior",
    "top": false
  },
  {
    "k": "goblin_5_crossties",
    "name": "GOBLIN. 5 CROSSTIES COFFEE 渋谷スクランブルスクエア店",
    "area": "Shibuya",
    "type": "coffee",
    "addr": "Shibuya Scramble Square 17F, 2-24-12 Shibuya, Shibuya-ku, Tokyo 150-0002 (direct Shibuya Station access)",
    "cap": "Room <b>1</b>seated 25 / Room 2 seated 19 / Room 1+2 seated 44 / standing 80; 127.55 m2 total",
    "flag": "dedicated",
    "link": "https://goblinspace.jp/space/goblin-shibuya-ssc",
    "linkText": "GOBLIN. official space page (5 CROSSTIES COFFEE) with spec + access",
    "email": null,
    "note": "Working Scramble Square 17F coffee hall configured as a rental floor — glass-walled with direct skyline views over the crossing, elevator-accessible, barrier-free, with 2 free mic/PA/projector units on 3F policies documented. Direct station access is the best logistics in this set for a Sept tech-week shuttle.",
    "img": null,
    "alt": "GOBLIN Scramble Square cafe interior",
    "top": false
  },
  {
    "k": "goblin_cafehotel_hotel",
    "name": "GOBLIN. 渋谷CAFE&HOTEL店 (HOTEL EMANON 南平台)",
    "area": "Shibuya",
    "type": "coffee",
    "addr": "HOTEL EMANON, 7-1 Nanpeidai-cho, Shibuya-ku, Tokyo 150-0036 (10 min walk from Shibuya Station)",
    "cap": "Total <b>220</b>across floors (1F 20 / 2F 40 / 3F 120 / #404 40); 3F alone 120 with projector, mic, PA, stage, MC podium all free",
    "flag": "dedicated",
    "link": "https://goblinspace.jp/space/shibuya-cafe-and-hotel",
    "linkText": "GOBLIN. official space page (渋谷CAFE&HOTEL) with full spec + 料金 table",
    "email": null,
    "note": "Hotel/penthouse-style cafe event house sized for the full 50–200 band: take just the 3F (120, fully AV-equipped) or stack floors toward 220 with catering at ¥5,500/person on-site. Whole-building buyout from ¥110,000/hour inc-tax, and the operator supplies staff on the day.",
    "img": null,
    "alt": "GOBLIN CAFE & HOTEL penthouse floor",
    "top": false
  },
  {
    "k": "shibuya_qws_scramble",
    "name": "SHIBUYA QWS Scramble Hall (渋谷キューズ スクランブルホール)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-6115 東京都渋谷区渋谷2-24-12 渋谷スクランブルスクエア15階 (Shibuya Scramble Square 15F, Shibuya QWS)",
    "cap": "Theater: <b>~200</b>/ School: ~90 / Party: ~100",
    "flag": "dedicated",
    "link": "https://venues.theatre-workshop.co.jp/scramblehall",
    "linkText": "Official event hall booking page (Theatre Workshop)",
    "email": "info@qws-scramblehall.jp",
    "note": "Sits inside SHIBUYA QWS, Tokyo's startup/innovation hub — directly hosts weekly tech pitch & demo events. Full AV/lighting/interpretation support with bilingual staff via Theatre Workshop. Tel 03-4221-4288.",
    "img": "https://venues.theatre-workshop.co.jp/hubfs/SCH_LP/facirity1.jpg",
    "alt": "Panoramic glass-walled hall overlooking Shibuya Crossing, theatre seating setup",
    "top": true
  },
  {
    "k": "shibuya_hikarie_8",
    "name": "Shibuya Hikarie 8/ COURT (渋谷ヒカリエ 8/COURT)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-8510 東京都渋谷区渋谷2-21-1 渋谷ヒカリエ8階 (Shibuya Hikarie 8F)",
    "cap": "Max <b>160</b>incl. staff; 172 sqm open space",
    "flag": "dedicated",
    "link": "https://space.hikarie8.com",
    "linkText": "Official 8/COURT rental site (Tokyu)",
    "email": null,
    "note": "Iconic green-carpet creative space inside Hikarie, 200-inch screen + full AV. Curated by the 8/ committee — screenings, exhibitions, talks, hack events. Committee approval required. Tel 03-6418-4718.",
    "img": "https://space.hikarie8.com/hubfs/hikarie8_2024/assets/images/home/kv.jpg",
    "alt": "Green-carpet open event space COURT on Hikarie 8F",
    "top": false
  },
  {
    "k": "sakura_deeptech_shibuya",
    "name": "SAKURA DEEPTECH SHIBUYA (サクラ ディープテック シブヤ)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-0031 東京都渋谷区桜丘町1-2 渋谷サクラステージ セントラルビル12階 (Shibuya Sakura Stage Central Bldg 12F)",
    "cap": "<b>150</b>(seated) / 200 (standing est)",
    "flag": "dedicated",
    "link": "https://www.sakuradeeptechshibuya.com/space/",
    "linkText": "SPACE page — event space, coworking, lab",
    "email": null,
    "note": "Purpose-built deeptech innovation hub run by Tokyu Land + Scrum Studio. Event space from ¥200k, food OK, open to public booking (not just members). Direct Shibuya Station New South Gate. Event-request form on /contact/.",
    "img": "https://www.sakuradeeptechshibuya.com/wp/wp-content/uploads/2026/06/event_01-1024x614.jpg",
    "alt": "Modern deeptech event space inside Shibuya Sakura Stage 12F",
    "top": false
  },
  {
    "k": "shibuya_cast_space",
    "name": "SHIBUYA CAST. SPACE & GARDEN (渋谷キャスト スペース・ガーデン)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-0002 東京都渋谷区渋谷1-23-21 渋谷キャスト GF (Shibuya CAST GF, Meiji-dori)",
    "cap": "Indoor: <b>121</b>(theater ~104 / seminar ~54 / standing 60-70); Outdoor Garden ~290sqm",
    "flag": "dedicated",
    "link": "https://eventspace.shibuyacast.jp/",
    "linkText": "Official event space site — SPACE + GARDEN rental",
    "email": null,
    "note": "Hybrid indoor 190sqm hall + 290sqm outdoor garden with bar counter; 190-inch LED display. Combined-use 20% discount. Operated by Theatre Workshop with bilingual coordination. 1 min from Shibuya Sta B1 exit. Tel 03-5778-9172.",
    "img": "https://eventspace.shibuyacast.jp/hubfs/SHIBUYA-CAST2022/assets/images/top/img-main01.jpg",
    "alt": "SHIBUYA CAST ground-floor event hall opening onto garden plaza",
    "top": false
  },
  {
    "k": "shibuya_solasta_conference",
    "name": "Shibuya Solasta Conference (渋谷ソラスタコンファレンス)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-0043 東京都渋谷区道玄坂1-21-1 渋谷ソラスタ 4階 (Shibuya Solasta 4F, Dogenzaka)",
    "cap": "Rooms <b>4</b>A/4D: 120 theater each; full-floor buyout up to ~240 across rooms + 200sqm lounge",
    "flag": "dedicated",
    "link": "https://shibuya.infield95.com",
    "linkText": "Official conference rental site",
    "email": null,
    "note": "Designer conference floor with 7 rooms (22-199sqm) + 200sqm shared designer lounge. NURO 10G internet, 120-inch projectors, full-day coordinators. Full buyout lets you run parallel tracks + reception — strong for a 150-200 person hackday. Tel 03-5784-2604.",
    "img": "https://shibuya.infield95.com/asset/media/images/home/variation_img01.jpg",
    "alt": "Solasta designer conference room and lounge interior",
    "top": false
  },
  {
    "k": "bloom_gate_at",
    "name": "BLOOM GATE at Shibuya Sakura Stage (ブルームゲート 渋谷サクラステージ)",
    "area": "Shibuya",
    "type": "cowork",
    "addr": "〒150-0031 東京都渋谷区桜丘町1-1 渋谷サクラステージ SHIBUYA TOWER 3F (Shibuya Sakura Stage, SHIBUYA side 3F)",
    "cap": "ZONE A: standing <b>60</b>/ seated 43; ZONE B: standing 40; ZONE C: standing 40 (combined 150-200 with all 3 zones + NIGIWAI STAGE)",
    "flag": "dedicated",
    "link": "https://www.shibuya-sakura-stage.com/en/media/eventspace/",
    "linkText": "Official event space page (4-language)",
    "email": null,
    "note": "Three linked zones + 675sqm outdoor NIGIWAI STAGE. Permanent W8.13m x H4.5m LED visions in zones A/B, kitchen in C. 4-language site incl. full English — easiest bilingual booking path. Direct from JR Shibuya Shinminami gate.",
    "img": "https://www.shibuya-sakura-stage.com/assets/images/media/eventspace/gage_zonea_img1.jpg",
    "alt": "BLOOM GATE open event floor with LED screen at Shibuya Sakura Stage",
    "top": false
  },
  {
    "k": "cafe_salvador_business",
    "name": "CAFE SALVADOR BUSINESS SALON (カフェ サルバドル ビジネスサロン)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "coffee",
    "addr": "103-0025 東京都中央区日本橋茅場町1-5-8 東京証券会館1階 (Kayabacho Station Exit 8, direct)",
    "cap": "<b>130</b>standing / 80 seated (whole-venue buyout); B meeting space up to 50, A up to 20",
    "flag": "dedicated",
    "link": "https://party.cafecompany.co.jp/stores/cafe-salvador-business-salon",
    "linkText": "CAFE SALVADOR BUSINESS SALON — party space page (Cafe Company)",
    "email": null,
    "note": "Cafe-meets-business-salon run by Cafe Company, built for exactly this crowd (entrepreneurs/investors in the old securities-exchange district). Published buyout for 51-130 with projector, mics, Wi-Fi, monitors; weekday full-venue minimum from JPY 360,000/3h. JP booking form on the party site; no published email, English not marked on the JP party site, so confirm bilingual handling by phone (03-5623-3105) or the inquiry form.",
    "img": "https://party.cafecompany.co.jp/wp-content/uploads/2025/12/th__U7A8933.jpg",
    "alt": "CAFE SALVADOR business salon counter",
    "top": true
  },
  {
    "k": "muromachi_cafe_hachi",
    "name": "muromachi cafe HACHI (ムロマチカフェハチ)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "coffee",
    "addr": "104-0032 東京都中央区日本橋室町4-4-10 東短室町ビルB1F (directly connected to Shin-Nihonbashi Exit 4; 2 min Mitsukoshimae)",
    "cap": "<b>150</b>standing / 90 seated (own site); buyouts from 60",
    "flag": "dedicated",
    "link": "https://hachi8.owst.jp/seats",
    "linkText": "muromachi cafe HACHI official site — seats & buyout (貸切) page",
    "email": null,
    "note": "True Nihonbashi cafe (breakfast-to-dinner) that actively promotes private hire with projector and mics; own site publishes buyout up to 150 standing/90 seated and its UI is localized in JP/EN/KO/ZH. Minimum guarantee around JPY 300,000 + JPY 50,000 venue fee per gnavi Wedding. Booking is JP-first (phone 03-3527-9108 or Hot Pepper); confirm English support when inquiring.",
    "img": "https://cdn.r-corona.jp/prd.rb.r-corona.jp/assets/site_files/fqxgiq4v/7341012/qsjr_w500h500.jpg",
    "alt": "muromachi cafe HACHI interior",
    "top": false
  },
  {
    "k": "two_rooms_cafe",
    "name": "TWO ROOMS CAFE GRILL BAR NIHOMBASHI",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "coffee",
    "addr": "103-0027 東京都中央区日本橋2-1-3 FUNDES日本橋2F (日本橋駅直結 / Nihonbashi Sta. connected)",
    "cap": "<b>250</b>standing exclusive hire / 100 dining-room standing / 70 seated; terrace 50",
    "flag": "dedicated",
    "link": "https://tworooms-nihombashi.jp/en/party/",
    "linkText": "TWO ROOMS NIHOMBASHI — Parties & Group Bookings (EN; JP toggle)",
    "email": null,
    "note": "The only venue here with a fully bilingual JA/EN party page (plans from JPY 6,000/person, terrace add-on) and up to 250 guests standing - easily covers a 200-person coffee-and-hack reception. Cafe-grill by day, strong AV staff; Nihonbashi Station direct connection. Definitely handles English inquiries.",
    "img": "https://tworooms-nihombashi.jp/images/page/party/party001.jpg",
    "alt": "Wide interior of TWO ROOMS Nihonbashi dining room and terrace set for a standing party",
    "top": false
  },
  {
    "k": "keshiki_aa_multipurpose",
    "name": "景色 Keshiki — AA (multipurpose space at the SR Coffee Roaster complex)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "coffee",
    "addr": "103-0026 東京都中央区日本橋兜町6-5 兜町第6平和ビル B1F-1F (2 min from Kayabacho Exit 11)",
    "cap": "<b>110</b>standing max / 60 seated (published)",
    "flag": "dedicated",
    "link": "https://mediasurf.co.jp/projects/keshiki",
    "linkText": "Keshiki project & AA space-rental page (Media Surf) with rates and contact email",
    "email": "info@mediasurf.co.jp",
    "note": "137sqm gallery-grade hall under the Kabutocho coffee-and-plant complex that houses SR Coffee Roaster's new flagship - the most on-brand backdrop for a specialty-coffee hack day. Published rates JPY 33,000/h (8:00-18:00), off-hours negotiable. JP site with published email; Media Surf hosts bilingual art events so EN handling is likely, confirm by email.",
    "img": "https://mediasurf.co.jp/wp-content/uploads/2024/02/WSL_4786.jpg",
    "alt": "Keshiki multipurpose space at SR Coffee Roasters",
    "top": false
  },
  {
    "k": "sikito_cafe_nihonbashi",
    "name": "SiKiTO CAFE Nihonbashi (シキトカフェ 日本橋)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "coffee",
    "addr": "103-0027 東京都中央区日本橋2-10-4 (2 min from Nihonbashi Station)",
    "cap": "<b>70</b>standing / 50 seated (published via OZmall listing; 59 seats on Tabelog); own page shows 3 bookable spaces",
    "flag": "dedicated",
    "link": "https://sikito.com/pages/cafe_party",
    "linkText": "SiKiTO CAFE official — 貸切・パーティー page (JP)",
    "email": null,
    "note": "Bright two-floor specialty cafe (1F counter + tables, 2F big tables) with dedicated party page listing Wi-Fi, mic, projector and screen for buyouts; works for a 50-70-person coffee track or overflow lounge next to a bigger hub. JP-first inquiry form plus chat on site; confirm English handling, cashless payment only.",
    "img": "https://sikito.com/cdn/shop/files/seminar_image_520x.png?v=1756368964",
    "alt": "Seminar-style setup inside SiKiTO CAFE Nihonbashi's bright two-floor cafe space",
    "top": false
  },
  {
    "k": "fingate_kayaba",
    "name": "FinGATE KAYABA (フィンゲート・カヤバ)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "cowork",
    "addr": "〒103-0025 東京都中央区日本橋茅場町1-8-1 茅場町一丁目平和ビル (1Fイベントスペース)",
    "cap": "<b>1</b>Fイベントスペース 100〜200名 (60坪 / 227.19㎡) — 定期開催のFinGATE Campus、Japan Fintech Week 2024の主要会場としても実績あり",
    "flag": "dedicated",
    "link": "https://www.fingate.tokyo/facility/kayaba/",
    "linkText": "Facility page with 1F event space details + contact form",
    "email": null,
    "note": "The standout pick. Japan's largest fintech/startup cluster in the Kabutocho-Kayabacho financial district; dedicated 1F event space fits 100–200 with coffee bar, FinGATE CLUB lounge on 3F, and bilingual (JP/EN) website + community manager. Frequently hosts 100+ person fintech, blockchain and startup events; the FSA and Fintech Association are tenants. High-speed Wi-Fi, projector, screen, sound, kitchen, and a rooftop-adjacent 8F shared lounge. Ideal for a Sept 9 tech-week coffee/hack day with bilingual attendees — they explicitly run English-language community events.",
    "img": "https://assets.st-note.com/img/1726796872-uyA24F89Bfozt0UwOmXiGrsI.jpg?width=1200",
    "alt": "FinGATE KAYABA event hall",
    "top": true
  },
  {
    "k": "muromachi_mitsui_hall",
    "name": "室町三井ホール＆カンファレンス (Muromachi Mitsui Hall & Conference)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "cowork",
    "addr": "〒103-0022 東京都中央区日本橋室町3-2-1 COREDO室町テラス3F",
    "cap": "Hall: シアター<b>420</b>席 / スクール306席 / 立食200名 (373㎡ 天井高5.9m) + 213㎡ foyer + Room A+B conference 108席",
    "flag": "dedicated",
    "link": "https://www.mitsui-mice.jp/nihonbashi/muromachi/",
    "linkText": "Official hall site with Floor / Pricing / Contact Form / EN page",
    "email": "muromachi-mitsuihall@mfbm.co.jp",
    "note": "The premium conference option. Direct connection to Mitsukoshimae (Metro) and Shin-Nihonbashi (JR) stations, 9-min walk from Tokyo Station. Recently hosted Metaverse Japan Summit, FIN/SUM (Nikkei × FSA fintech conference), HIMSS Health 2.0 — proven for 300+ tech/bilingual crowds. NURO Premium dedicated 2Gbps line (bandwidth-guaranteed) included free, in-house catering, English contact page. Foyer (100 standing) could serve as coffee/networking zone while Hall hosts talks + hack demos. Bookable 1 year ahead, so Sept 9 2026 must be locked in by ~Sept 2025 —— reserve now.",
    "img": "https://www.mitsui-mice.jp/nihonbashi/wp/wp-content/themes/mitsui-mice/assets/img/pic/floormap.png",
    "alt": "Muromachi Mitsui Hall concourse",
    "top": false
  },
  {
    "k": "businessairport_nihonbashi",
    "name": "ビジネスエアポート日本橋 (Business-Airport Nihonbashi)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "cowork",
    "addr": "〒103-0027 東京都中央区日本橋3-6-2 日本橋フロント1F",
    "cap": "イベントスペース 約<b>80</b>㎡ / シアター形式 約80名 (会員33,000円/時、非会員49,500円/時) + 会議室A 8名 + B 4名 + C 6名",
    "flag": "dedicated",
    "link": "https://business-airport.net/shop/nihonbashi/",
    "linkText": "Nihonbashi location page with event-space facility section, pricing, and dedicated 見学予約/お問い合わせ forms",
    "email": null,
    "note": "Most coffee-shop-like of the bunch: warm wood 'Memory of the Bridge' interior on the ground floor of a street-front building — feels like a café, not a conference room, perfect for the 'coffee/hack day' mood. Tokyu Fudosan-run with bilingual staff and bilingual web booking. 2 min from Nihonbashi Station B1 exit / 5 min from Tokyo Station Yaesu. Regularly hosts its own startup meetups, marketing night, product night — staff are used to tech-crowd events. Wi-Fi, power, phone booths, on-site konbini, projector loan. Cap of ~80 means it fits the lower end of 50-200; pair with FinGATE for overflow.",
    "img": "https://business-airport.net/wordpress/wp-content/uploads/2025/09/nihonbashi-gallery_img09.jpg",
    "alt": "Business-Airport Nihonbashi lounge",
    "top": false
  },
  {
    "k": "xbridgeyaesu",
    "name": "xBridge-Yaesu (クロスブリッジヤエス)",
    "area": "Nihonbashi / Kayabacho (Chūō)",
    "type": "cowork",
    "addr": "〒103-0028 東京都中央区八重洲1-5-20",
    "cap": "コワーキング約<b>44</b>席; 夜間イベントスペース貸切 ~60名 (立食) / 着席約50名 — プロジェクター・スクリーン・音響込み ¥20,000/時",
    "flag": "dedicated",
    "link": "https://xbridge.tokyo/",
    "linkText": "Top page with facilities, event rental terms and 'EVENT' contact section",
    "email": null,
    "note": "Run by Tokyo Tatemono with on-site VC (XTech Ventures). Explicitly marketed for evening pitch events, networking and study sessions — bilingual staff accustomed to global startup community (the xBridge-Global sister site has an English café, GOOD COFFEE FARMS, run by Guatemalan founder Carlos Melen). 3 min from Nihonbashi Station A3/B3 (Metro Ginza/Tozai/Asakusa), 3 min from Tokyo Station Yaesu north. Sized for the small end (50-60) but the most 'tech-week' vibe per square meter, with a real coffee bar at the entrance.",
    "img": null,
    "alt": "xBridge-Yaesu open lounge",
    "top": false
  },
  {
    "k": "hills_cafspace_roppongi",
    "name": "Hills Café/Space (ヒルズ カフェ/スペース) — Roppongi Hills",
    "area": "Roppongi",
    "type": "coffee",
    "addr": "〒106-0032 東京都港区六本木6-10-1 六本木ヒルズ ヒルサイド 2F",
    "cap": "Seated <b>155</b>/ Standing ~200 (published on HILLS MEDIA & SPACE spec: 店内約290㎡, 催事有効約220㎡)",
    "flag": "dedicated",
    "link": "https://www.hillsmediaspace.com/space_rh_detail03.html",
    "linkText": "Mori Building HILLS MEDIA & SPACE — dedicated Hills Café/Space event-rental page (price, spec, booking flow, FLOW 01-07)",
    "email": "hillsbfast@gmail.com",
    "note": "True café with terrace inside Roppongi Hills, run by Mori Building's media/events arm — the link above is the venue-operator's own rental page with full spec, rate card and application flow. JP booking path is fully published (content review → 申込書 → 50% deposit); an English twin page (/en/space_rh_detail03.html) also fetched successfully, and Mori Building's team routinely handles international/bilingual tech and brand events at the Hills complex. Phone 03-6406-6666 (Mori Media Business Planning) — form-only otherwise. (Contact is via Hills Café breakfast-hotline mailbox hillsbfast@gmail.com — nonstandard channel for event bookings; phone recommended.)",
    "img": "https://transithospitalitymanagement.com/_wp/wp-content/uploads/2024/10/01-1-64.jpg",
    "alt": "Wide interior/patio shot of the Hills Café/Space event floor inside Roppongi Hills",
    "top": true
  },
  {
    "k": "park6_powered_by",
    "name": "PARK6 powered by bondolfi boncaffè (パークシックス) — Roppongi Hills West Walk 6F",
    "area": "Roppongi",
    "type": "coffee",
    "addr": "〒106-6108 東京都港区六本木6-10-1 六本木ヒルズ森タワー ウェストウォーク6F",
    "cap": "Seated <b>~100</b>/ Standing ~200 (est) — venue's own floor plan and partner listings; 30名〜150名目安 published on partner page",
    "flag": "contact",
    "link": "http://www.park6.jp",
    "linkText": "PARK6 official site (CAFE+WORK LOUNGE). 貸切・場所貸し・撮影利用 section links to the Coubic booking form (https://coubic.com/park6_RESERVATION)",
    "email": null,
    "note": "Café+work-lounge on Roppongi Hills' 6th floor, run by Transit Hospitality and branded with Italian roaster bondolfi boncaffè — park-like, plant-filled, with outlets and Wi-Fi everywhere, so it feels exactly like a 'coffee + hack day' room. Weekday café mode runs 10:00-19:00; 貸切/場所貸し/撮影利用 is offered for evenings and weekends (Sun/holidays are otherwise closed, so Sept 9 weekday party likely means an evening buy-out). Phone 03-6434-0546; booking via Coubic (JP UI) — English support typical at Roppongi Hills.",
    "img": "https://www.park6.jp/img/about.jpg",
    "alt": "Park-like café + work lounge interior at PARK6 Roppongi Hills",
    "top": false
  },
  {
    "k": "ark_hills_cafe",
    "name": "ARK HiLLS CAFE (アークヒルズ カフェ) — ARK Hills, Roppongi-itchome",
    "area": "Roppongi",
    "type": "coffee",
    "addr": "〒106-0032 東京都港区六本木1-3-40 アークヒルズ カラヤン広場 スペイン坂入り口",
    "cap": "Seated <b>70</b>/ Standing 150; floor can be configured for up to 200 (per venue listing)",
    "flag": "dedicated",
    "link": "https://kasikiru.com/detail/340",
    "linkText": "カシキル (kasikiru) — dedicated ARK HiLLS CAFE rental page with 2026-09 availability calendar, plan pricing, equipment list, cancellation policy",
    "email": null,
    "note": "Standalone café at the Karajan Place entrance of ARK Hills facing Spain-zaka — a real, day-to-day café that also runs a full buy-out programme. Open terrace, movable furniture, big rollable projector screen + 2 wireless mics + dedicated sound and a green room; 1 min from Roppongi-itchome Stn (Namboku Line). One floor, fully private when rented. Phone 03-6229-2666 (café) or kasikiru concierge 03-6732-3904 (bilingual-concierge style support); listed as English-friendly in tourist-facing listings.",
    "img": "https://cdn.kasikiru.com/upload/photos/73746f72655f333430/gallery/ins7eqpund.jpg",
    "alt": "Open-plan Café interior at ARK HiLLS CAFE with terrace access",
    "top": false
  },
  {
    "k": "caf_deli_ggco",
    "name": "Café & Deli GGCo. 城山 (ジージーコー シロヤマ) — Shiroyama Trust Tower, Toranomon/Roppongi-itchome edge",
    "area": "Roppongi",
    "type": "coffee",
    "addr": "〒105-6001 東京都港区虎ノ門4-3-1 城山トラストタワー 1F",
    "cap": "Seated <b>80</b>/ Standing 100 (listing says up to 120 possible when fully arranged)",
    "flag": "dedicated",
    "link": "https://kasikiru.com/detail/1079",
    "linkText": "カシキル (kasikiru) — dedicated GGCo. Shiroyama rental page with capacity, equipment, layout notes",
    "email": null,
    "note": "Urban café operated by Mori Trust Hotels & Resorts, inside the Shiroyama Trust Tower lobby — high ceilings, bright natural light, two big screens, pro audio, designer furniture that can be rearranged for lectures or hack sessions. 6-8 min from Roppongi-itchome Stn (Namboku) or 3 min from Kamiyacho (Hibiya), so it's on the Roppongi fringe but very 'tech-week' practical. Rental desk runs through kasikiru (JP); venue phone 03-6435-6443. Group runs 4 GGCo. cafés across Tokyo; English handling via hotel-group staff is standard.",
    "img": "https://cdn.kasikiru.com/upload/photos/73746f72655f31303739/GGco_%E5%86%85%E8%A6%B3a.jpg",
    "alt": "Daylight, high-ceiling interior of Café & Deli GGCo. Shiroyama",
    "top": false
  },
  {
    "k": "kafli_roppongi_nogizaka",
    "name": "KAFLI (カフリ) — Roppongi / Nogizaka",
    "area": "Roppongi",
    "type": "coffee",
    "addr": "〒106-0032 東京都港区六本木 (centre of Roppongi/Nogizaka; station access: Chiyoda Line Nogizaka 5 min, Hibiya/Oedo Roppongi 8 min)",
    "cap": "Seated <b>60</b>(listing also notes up to 112 configured) / Standing 150 (one-floor buy-out)",
    "flag": "dedicated",
    "link": "https://kaijosearch.com/kanto/F03564/0",
    "linkText": "会場ベストサーチ (kaijosearch) — dedicated KAFLI rental listing with Sept-2026 availability calendar, plan inquiry form, capacity table",
    "email": null,
    "note": "Café-lounge styled single-floor rental space between Roppongi and Nogizaka with an ivory interior, atrium, and classic chandeliers — more 'stylish café lounge' than bar-club, and it has DJ booth, sound/vision kit and a small stage that doubles well for talks. Operator works with both JP corporate parties and music/culture events, accustomed to international crowds in this district. Booking via kaijosearch form (JP) — venue staff reachable through the form for both JP and EN inquiries.",
    "img": null,
    "alt": "Atrium café-lounge interior at KAFLI Roppongi",
    "top": false
  },
  {
    "k": "blink_roppongi_blink",
    "name": "BLINK Roppongi (BLINK 六本木)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "3-1-6 Motoazabu, Minato-ku, Tokyo 106-0046 (東京都港区元麻布3-1-6)",
    "cap": "<b>~100</b>(est) cafe lounge whole-floor event rental; published hourly rate ¥16,500/h (tax incl.)",
    "flag": "dedicated",
    "link": "https://blinkcommunity.com/en/service/",
    "linkText": "Space Rental",
    "email": null,
    "note": "Bilingual JA/EN staff and a fully bilingual booking site. 1F cafe lounge is rented as an event space (events, seminars, workshops, filming), open to non-members, with on-site coffee bar — perfect coffee/hack-day vibe. Hosts 200+ events/year; easy EN email flow via contact form.",
    "img": "https://blinkcommunity.com/wp-content/themes/blink/assets/images/space/sp-detail-floor-img.jpg",
    "alt": "BLINK Roppongi 1F international cafe lounge / event space",
    "top": true
  },
  {
    "k": "bellesalle_roppongi_grand",
    "name": "Bellesalle Roppongi Grand Conference Center (ベルサール六本木グランドコンファレンスセンター)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "Sumitomo Fudosan Roppongi Grand Tower 9F, 3-2-1 Roppongi, Minato-ku, Tokyo 106-6209 (東京都港区六本木3-2-1 住友不動産六本木グランドタワー9F)",
    "cap": "Up to <b>456</b>theater (Room H+I, 516 m2); divisible into 9 rooms of 32–340 seats for smaller 50–200-person formats",
    "flag": "dedicated",
    "link": "https://www.bellesalle.co.jp/shisetsu/roppongi/bs_roppongi_cc/",
    "linkText": "Bellesalle Roppongi Grand Conference Center",
    "email": "bs-info@j.sumitomo-rd.co.jp",
    "note": "Purpose-built conference floor directly connected to Roppongi-itchome Station west gate, with dedicated elevator for private-floor events. Publishes hourly rates, seat layouts, 360° tour. EN-facing 'For overseas corporations' note: inquiries by email, English OK on email; venue staff JP-only so budget an interpreter.",
    "img": "https://www.bellesalle.co.jp/wp-content/uploads/00ea6c3f9b83e6c0ad9b5b68b1d19c616a054d8e.jpg",
    "alt": "Bellesalle Roppongi Grand Conference Center 9F conference floor",
    "top": false
  },
  {
    "k": "izumi_garden_gallery",
    "name": "Izumi Garden Gallery (泉ガーデンギャラリー)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "Izumi Garden, 1-5-2 Roppongi, Minato-ku, Tokyo 106-0032 (東京都港区六本木1-5-2 泉ガーデンギャラリー)",
    "cap": "Up to <b>720</b>theater / 490 in Gallery A / 240 in Gallery B; total 883 m2",
    "flag": "dedicated",
    "link": "https://www.bellesalle.co.jp/shisetsu/roppongi/izumig_gallery/",
    "linkText": "Izumi Garden Gallery",
    "email": "bs-info@j.sumitomo-rd.co.jp",
    "note": "Standalone event hall on the green Izumi Garden grounds, 3 min walk from Roppongi-itchome. 6 m ceilings, 5x 250-inch screens, dedicated line — proven for embassy + global tech events (IPEF launch, Nijisanji live viewings). Same Sumitomo Bellesalle team; EN via email, JP-only on-site.",
    "img": "https://www.bellesalle.co.jp/wp-content/uploads/izumigg_building_hall_8013.jpg",
    "alt": "Izumi Garden Gallery interior, 6 m ceiling exhibition hall",
    "top": false
  },
  {
    "k": "cirq_roppongi",
    "name": "CIRQ Roppongi (シルク六本木)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "Palazzo Ducale Azabu, 7-12-27 Roppongi, Minato-ku, Tokyo 106-0032 (東京都港区六本木7-12-27 パラッツォ ドゥカーレ 麻布)",
    "cap": "La Foresta (B1, <b>165</b>m2): 100 standing / 96 seated / 80 theater; La Terrazza (1F, 120 m2): 80 seated / 77 theater",
    "flag": "dedicated",
    "link": "https://www.cirq-hall.jp/facility/cirq-roppongi",
    "linkText": "CIRQ Roppongi",
    "email": null,
    "note": "TKP-operated dedicated event hall 2 min from Roppongi Sta, with rescindable 5-business-day provisional hold, VR tours of both rooms and published floor layouts — well-suited for a 50–100-person hack/coffee day. TKP has an English corporate site; JP-primary on booking flow.",
    "img": "https://www.cirq-hall.jp/wp-content/uploads/2020/09/plazzo_ducale_la_foresta.jpg",
    "alt": "CIRQ Roppongi La Foresta hall, B1",
    "top": false
  },
  {
    "k": "bellesalle_roppongi",
    "name": "Bellesalle Roppongi (ベルサール六本木)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "Roppongi, Minato-ku, Tokyo 106-0032 (bellesalle official page; building detail on request)",
    "cap": "<b>1</b>F Hall A 84 / Hall B 90 / combined whole-floor 1F Hall up to ~300 (est); plus B1 hall",
    "flag": "dedicated",
    "link": "https://www.bellesalle.co.jp/shisetsu/roppongi/bs_roppongi/",
    "linkText": "Bellesalle Roppongi",
    "email": "bs-info@j.sumitomo-rd.co.jp",
    "note": "Sister Sumitomo Bellesalle hall 2 min from Roppongi Sta exit 2. Smaller sister of the Grand Conference Center, useful for mid-size tech meetups / 50–200 people. Same EN email on the 'For overseas corporations' panel; do not confuse with the Grand CC address.",
    "img": "https://www.bellesalle.co.jp/wp-content/uploads/keyvisual_bs_roppongi.png",
    "alt": "Bellesalle Roppongi 1F and B1 hall",
    "top": false
  },
  {
    "k": "azabuballoon",
    "name": "AZABUballoon (麻布バルーン)",
    "area": "Roppongi",
    "type": "cowork",
    "addr": "Azabu-Juban vicinity, Minato-ku, Tokyo (詳細な住所・内見は問合せフォームから)",
    "cap": "<b>100</b>standing / 80 seated; 202.51 m2 (61.25 tsubo)",
    "flag": "dedicated",
    "link": "https://azabuballoon.com/party",
    "linkText": "AZABUballoon",
    "email": null,
    "note": "Independent live/event space 4 min from Azabu-Juban, 8 min from Akabanebashi — short hop from Roppongi. Pro audio/lighting, 6.6 m stage, onsite kitchen and bar; ok on the smaller end for a 50–100-person casual coffee/hack day. JP-primary site — partner with a JP speaker to book.",
    "img": "https://azabuballoon.com/wp-content/uploads/azabu_zumen1.jpg",
    "alt": "AZABUballoon event space, stage + counter bar",
    "top": false
  }
];